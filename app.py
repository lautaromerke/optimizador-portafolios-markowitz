...
if st.sidebar.button("🚀 Ejecutar Planificación Integral"):
    with st.spinner("Procesando modelos cuantitativos y proyecciones..."):
        try:
            ...
            # Simulación Montecarlo Markowitz Básica
            ...
            # Optimización por objetivo numérico con Scipy
            restricciones = (
            ...
            # Mapeo de la cartera elegida
            if opt_res.success:
                pesos_finales = opt_res.x
                nombre_cartera = "Cartera Objetivo"
            else:
                pesos_finales = best_w_sharpe
                nombre_cartera = "Cartera Máx. Sharpe"

            # Muestra de métricas en pantalla
            ...
            with c2:
                if opt_res.success:
                    ...
                    st.write("⚠️ Meta inalcanzable. Se optó por Máx. Sharpe.")

            with c3:
                st.write(f"### Composición de la {nombre_cartera} (%)")
            ...
            # --- PROYECCIÓN DE MONTECARLO / RETIRO ---
            st.markdown("---")
            ...
            with col_met2:
                st.markdown(f"💡 **Análisis Cuantitativo:** Hay un **{prob_exito:.1%}** de probabilidades de que tus fondos resistan retiros de **USD {retiro_mensual:,.2f}/mes** durante {anios_proyeccion} años.")
