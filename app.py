...
# =========================================================================
# NUEVO MÓDULO: BLACK-LITTERMAN + PRESETS DINÁMICOS
# =========================================================================
if modulo == "🔮 Algoritmo Black-Litterman (Opinión + Presets)":
    ...
    # 1. Selector de Escenarios (Punto 1)
    preset = st.selectbox(
    ...
    for idx, col_name in enumerate(datos_usd.columns):
        # Valores por defecto basados en el Preset elegido
        default_v = 0.0
        default_c = 50
    ...
    if st.button("🔮 Ejecutar Fusión Bayesiana"):
        ...
        if res_bl.success:
            ...
            st.session_state['cartera_optima'] = df_bl # Guardamos para el módulo de rebalanceo
...
# =========================================================================
# NUEVO MÓDULO: DIAGNÓSTICO Y REBALANCEO AUTOMÁTICO (Punto 2)
# =========================================================================
    ...
    # Parámetros financieros del rebalanceo
    ...
    if abs(sum_pesos - 100.0) > 0.1:
        ...
        if st.button("⚡ Analizar y Corregir Cartera"):
            # Si calculó Black-Litterman antes usa esa, si no calcula una Markowitz rápida de referencia
            if 'cartera_optima' in st.session_state:
                df_target = st.session_state['cartera_optima']
                origen_target = "Sugerida por Black-Litterman"
            else:
                # Fallback: Max Sharpe histórico rápido
            ...
            # Combinación y cálculo de desvíos financieros
            df_reb = pd.DataFrame({
            ...
            # Gráfico de comparación visual de barras
            ...
            # Tabla de Ajustes con Plan de Acción Inteligente
...
# =========================================================================
# MÓDULOS ANTERIORES CONSERVADOS EN SU TOTALIDAD (MÉTODOS TRADICIONALES)
# =========================================================================
    ...
    if st.button("⏳ Correr Análisis"):
        ...
        st.metric("Probabilidad de éxito del fondo", f"{np.sum(simulations[-1] > 0) / 100:.1%}")import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
