...
# Menú sin emojis para evitar errores de sintaxis
...
if modulo == "Black-Litterman":
    ...
    st.write("Optimización avanzada.")
    ...
    st.header("💰 Simulador de Supervivencia de Capital")
    c1, c2 = st.columns(2)
    capital_inicial = c1.number_input("Capital Inicial (USD):", value=50000, step=5000)
    retiro_mensual = c2.number_input("Retiro Mensual (USD):", value=500, step=50)
        ...
        for nombre, params in perfiles.items():
            ...
            for s in range(30):
                for t in range(1, meses):
                    ...
                    trayectorias[t, s] = max(0, trayectorias[t-1, s] * (1 + rend) - retiro_mensual)
            df_sim[nombre] = np.mean(trayectorias, axis=1)
        st.line_chart(df_sim)
        
        c1, c2, c3 = st.columns(3)
        for i, nombre in enumerate(perfiles.keys()):
            meses_vida = (df_sim[nombre] > 0).sum()
            [c1, c2, c3][i].metric(f"Perfil {nombre}", f"{meses_vida} meses", f"{meses_vida/12:.1f} años")     
            duracion_meses = (df_sim[nombre] > 0).sum()
